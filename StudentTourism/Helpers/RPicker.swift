...
@objc open class RPicker: NSObject {
    ...
    */
    ...
    class func selectOption(title: String? = nil,
                            ...
                            didSelectValue : ((_ value: String, _ atIndex: Int)->())?, onWillDismiss: VoidBlock? = nil)  {
        
        guard let arr = dataArray, let vc = controller(title: title, cancelText: cancelText, doneText: doneText, dataArray: arr, selectedIndex: selectedIndex, type: .option, onWillDismiss: onWillDismiss) else { return }
    ...
    private class func controller(title: String? = nil,
        ...
        if let cc = UIWindow.currentController {
            if RPicker.sharedInstance.isPresented == false {
                ...
                cc.present(vc, animated: false) {
                    UIApplication.keyWindow?.addSubview(vc.view)
                    UIApplication.keyWindow?.bringSubviewToFront(vc.view)
                }
                vc.onWillDismiss = {
                    onWillDismiss?()
...
private extension UIView {
    ...
    func pinConstraints(_ byView: UIView, left: CGFloat? = nil, right: CGFloat? = nil, top: CGFloat? = nil, bottom: CGFloat? = nil, height: CGFloat? = nil, width: CGFloat? = nil) {
        ...
        if let b = bottom { bottomAnchor.constraint(equalTo: byView.safeAreaLayoutGuide.bottomAnchor, constant: b).isActive = true }
...
class RPickerController: UIViewController {
    ...
    private func initialSetup() {
		...
		bgView.addSubview(whiteViewBack)
		whiteViewBack.pinConstraints(bgView, left: 0, right: 0, bottom: 60, height: 130)
        ...
        stackView.pinConstraints(view, left: 0, right: 0, bottom: 30, height: height)
    ...
    private func setUpThemeMode(isDark: Bool) {
        ...
        if isDark {
			...
			whiteViewBack.backgroundColor = UIColor.black
            if pickerType == .date {
			...
			whiteViewBack.backgroundColor = UIColor.white
	...
	private lazy var whiteViewBack: UIView = {
		let v = UIView()
