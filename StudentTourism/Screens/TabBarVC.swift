...
final class TabBarVC: UITabBarController, UITabBarControllerDelegate {
    ...
    init() {
        ...
        let eventsVC = MainVC.instantiate()!
        eventsVC.tabBarItem = UITabBarItem(title: nil, image: Asset.Assets.favTab.image, tag: 1)
        let contactsVC = MainVC.instantiate()!
        contactsVC.tabBarItem = UITabBarItem(title: nil, image: Asset.Assets.bookTab.image, tag: 2)
        let wishList = MainVC.instantiate()!
        wishList.tabBarItem = UITabBarItem(title: nil, image: Asset.Assets.notifyTab.image, tag: 3)
        let chatsVC = MainVC.instantiate()!
        chatsVC.tabBarItem = UITabBarItem(title: nil, image: Asset.Assets.profileTab.image, tag: 4)
        
        viewControllers = [ UINavigationController(rootViewController: mainVC),
                            UINavigationController(rootViewController: wishList),
